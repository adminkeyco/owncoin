...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = _("Keyco Core keyco-tx utility version") + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  keyco-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded keyco transaction") + "\n" +
              "  keyco-tx [options] -create [commands]   " + _("Create hex-encoded keyco transaction") + "\n" +
              "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded keyco transaction
